...
.. TODO: в заголовке документа должен быть Python
   TODO: больше примеров форматированного вывода
   TODO: про dir рассказывается позже, чем он начинает использоваться
   TODO: пакеты отличаются от модулей; например, пакеты нельзя запустить через
         `python -m'
...
* ``zip(iter1[, iter2[, ...]])``
  ...
  - возвращает последовательность кортежей:
    ...
    - (вторые элементы последовательностей)
    - (третьи элементы последовательностей)
    - ...
...
* ``map(func, iter1[, iter2[, ...]])`` — применить ``func`` к каждому элем. посл.
  ...
  - возвращает последовательность:
    ...
    - func(вторые элементы последовательностей)
    - func(третьи элементы последовательностей)
    - ...
